...
class Menu:
    ...
    def change_active(self, item, a):
        for i in range(len(self.items)):
            if self.items[i][0] == item:
                self.items[i] = (self.items[i][0], self.items[i][1],
                                 self.items[i][2], a)
    ...
    def up(self):
        if len(self.items) == 0:
            return
        self.selected -= 1                          # FIX: uniform indentation
        if self.selected < 0:
            self.selected = len(self.items) - 1
        if not self.items[self.selected][3]:
            self.up()
    ...
    def select(self):
        if self.items[self.selected][1]:            # FIX: was bare `selected` (NameError)
            self.items[self.selected] = (
                self.items[self.selected][0],
                self.items[self.selected][1],
                not self.items[self.selected][2],
    ...
    def draw(self):
        ...
        for i in range(n):
            ...
            if self.items[i][1] == 1:
                offset = 35
                result.blit(Settings.box,
                            (25, int(2.5 * Settings.MENU_LINEFEED)
                             + Settings.MENU_LINEFEED * i - 8))
                if self.items[i][2]:
                    if self.items[i][3]:
                        result.blit(Settings.tick,
                                    (25, int(2.5 * Settings.MENU_LINEFEED)
                                     + Settings.MENU_LINEFEED * i - 8))
                    else:
                        result.blit(Settings.tick_inactive,
                                    (25, int(2.5 * Settings.MENU_LINEFEED)
                                     + Settings.MENU_LINEFEED * i - 8))
            else:
                offset = 0
            result.blit(txt,
                        (25 + offset,
                         int(2.5 * Settings.MENU_LINEFEED)
                         + Settings.MENU_LINEFEED * i))

        pygame.draw.rect(result, (0, 0, 200),
...
class Numeric(Menu):
    ...
    def draw(self):
        ...
        rect = txt.get_rect()
        rect.midtop = (w // 2, int(2.5 * Settings.MENU_LINEFEED))
        result.blit(txt, rect.topleft)
...
class Confirm(Menu):
    ...
    def draw(self):
        ...
        for i in range(2):
            ...
            rect = txt.get_rect()
            if i == 0:
                rect.topright = (w // 2 - Settings.MENU_LINEFEED,
                                 3 * Settings.MENU_LINEFEED + offset)
            else:
                rect.topleft = (w // 2 + Settings.MENU_LINEFEED,
                                3 * Settings.MENU_LINEFEED + offset)
            result.blit(txt, rect.topleft)
